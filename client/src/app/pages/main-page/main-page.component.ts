...
export class MainPageComponent implements OnInit, OnDestroy {
  ...
  gameLoop = () => {
    ...
    for (let y = 0; y < this.mapHeight; y++) {
      for (let x = 0; x < this.mapWidth; x++) {
        ...
        this.ctx.lineTo(
          screenX + this.tileSizeX / 2,
          screenY + this.tileSizeY / 4
        );
    ...
    const characterScreenX =
      ((this.characterX - this.characterY) * this.tileSizeY) / 2 +
      this.gameCanvas.nativeElement.width / 2;
    const characterScreenY =
      ((this.characterX + this.characterY) * this.tileSizeX) / 4 -
      this.tileSizeY / 2;
    this.ctx.drawImage(
      this.characterImage,
      characterScreenX,
  ...
  handleKeyDown = (event: KeyboardEvent) => {
    if (event.code === 'ArrowUp' && this.characterY > 0) {
      ...
      this.characterY < this.mapHeight - 1
    ) {
      this.characterY++;
      ...
      this.characterX < this.mapWidth - 1
    ) {
      this.characterX++;
    }
  };
